...
def perdelta(start, end, delta):
  """
    ...
    Method receives "start" and "end" as "dates" and returns dates between them with interval specified in "delta".
  """
...
def convert_date(date_to_convert):
  """
    ...
    Method receives parameter "date_to_convert" in format 'yyyyddmm' (for example '20130101') and converts it to the int.
  """

  return int(date_to_convert[:4]), int(date_to_convert[4:6]), int(date_to_convert[6:])

def get_gdelt_daily_updates_from_date(directory, from_date, to_date=None, unzip_dir = '../unzipped/'):
  """
    (str, date) -> NoneType
  ...
  for temp_date in perdelta(date(from_year, from_month, from_day), date(to_year, to_month, to_day), timedelta(days=1)):
    ...
    if not os.path.isfile( unzip_dir + '/' + file_name):
        written_file = _download_chunks(directory, get_url)
        _unzip_file(unzip_dir, written_file)
...
if __name__ == '__main__':
    ...
    fetch_command.add_argument('-d', '--directory', help="""Path of directory for file download""")

    fetch_command.add_argument('-U', '--unzip', action='store_true',
                               default=False,
                               help="""Boolean flag indicating whether
                                or not to unzip the downloaded
                                files.""")
    ...
    fetch_from_date_command.add_argument('-du', '--directoryunzip',
                                        help="""Path of directory to unzip""")
    ...
    fetch_from_date_command.add_argument('-T', '--todate',
                                        default=False, 
                                        help="""Specify 'to date',
                                        if not specified 'current date' is taken as 'end date'.""")
    ...
    if args.command_name == 'fetch':
      ...
      unzip = args.unzip
      unzip_directory = args.directoryunzip
      now = datetime.date.today()-datetime.timedelta(days=1)

      if args.fromdate == False:
        get_gdelt_daily_updates_from_date(directory, now.strftime("%Y%m%d"), unzip_dir = unzip_directory)
      elif args.todate == False:
        get_gdelt_daily_updates_from_date(directory, args.fromdate, unzip_dir = unzip_directory)
      else:
        get_gdelt_daily_updates_from_date(directory, args.fromdate, args.todate, unzip_dir = unzip_directory)
